...
public class CustomExpandableListAdapter extends BaseExpandableListAdapter {
	...
	public void reInit() {
		DBHandler dbHandler = new DBHandler(context);
		List<ListObject> databaseList = dbHandler
				.getAllListObjectsThatHaveTimeAndCategories();

		List<ListObject> totalList = new ArrayList<ListObject>();
		totalList.addAll(databaseList);
		// TODO Add removal of old events

		Category autogenCategory = dbHandler.getCategory(context
				.getString(R.string.autogenerated));
		...
		if (blocks != null && !blocks.isEmpty()) {
			...
			for (ListObject lo : autoGeneratedList) {
				lo.addToCategory(autogenCategory);
			}
			if (!autoGeneratedList.isEmpty()) {
				addCategory(autogenCategory);
			}
		...
		for (ListObject lo : totalList) {
			...
			if (lo.getTime() != null) {
				Log.d("ListObjectToAdd", "" + lo.getName() + ", cat: "
						+ lo.getCategories().toString());
	...
	public View getChildView(int groupPosition, final int childPosition,
		...
		if (getLOShouldBeVisible(listObject)) {
			...
			if (listObject.getTime() != null) {
				if (somedayEventList.contains(listObject) || thisWeekEventList.contains(listObject)) {
			...
			childStartTimeView.setText(childStartTimeText);
			...
			if (listObject.isImportant())
				ribbonColor.setColor(Color
						.parseColor("#"
                                + listObject.getCategories().get(0)
                                .getImportantColor()));
			else
				ribbonColor.setColor(Color.parseColor("#"
						+ listObject.getCategories().get(0).getBaseColor()));
	...
	/**
	 * <<<<<<< HEAD <<<<<<< HEAD removes a list with categories from the list
	 * with categories displayed in by this adapter. ======= removes a list with
	 * categories from the list with categories displayed by this adapter.
	 * >>>>>>> dev ======= removes a list with categories from the list with
	 * categories displayed in by this adapter. >>>>>>> autogenerator.generate
	 * 
	 * @param catList
